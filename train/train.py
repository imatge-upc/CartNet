...
from train.metrics import compute_metrics_and_logging
...
def train(model, loaders, optimizer, loggers):
    ...
    run = wandb.init(entity="aiquaneuro", project=cfg.wandb_project,
                    name=cfg.name)
    ...
    for cur_epoch in range(cfg.optim.max_epoch):
        ...
        run.log(flatten_dict(perf[:-1]), step=cur_epoch)

        
        # Log current best stats on eval epoch.     
        best_epoch = np.array([vp["MAE"] for vp in perf[1]]).argmin()
        ...
        for i, s in enumerate(['train', 'val']): 
            ...
            if m in perf[i][best_epoch]:
                bstats[f"best/{s}_{m}"] = perf[i][best_epoch]["MAE"]
        logging.infor(bstats)
    ...
    eval_epoch(loggers[-1], loaders[-1], model, split="test", test_metrics=True)
